...
const getChildDataWeight = async (childDataWeight) => {
  try {
    console.log("Child Data Weight:", childDataWeight); // Log the childDataWeight object
    ...
    const { weight, childDateOfBirth, childGender, childObjectId } = childDataWeight;
    const dateOfBirth = new Date(childDateOfBirth); // Convert childDateOfBirth to Date object
    const currentDate = new Date(); // Get the current date

    // Construct the request payload
    const measurementRequest = {
      birth_date: dateOfBirth.toISOString().slice(0, 10), // Convert to ISO string
      observation_date: currentDate.toISOString().slice(0, 10), // Convert to ISO string
