...
const  ProfMessagesScreen = ({ route }) => {
  const { objectId ,receiverId} = route.params;
      ...
      try {
        if (senderObjectId && receiverId) {
          ...
          const [leftBubbleMessages, fetchedImageUrlsData] = await Promise.all([
            fetchLeftBubbleMessages(receiverId, senderObjectId),
            fetchImageUrls(receiverId, senderObjectId),
          ]);
  ...
  const openPhotoPicker = async () => {
    try {
      await AsyncStorage.setItem(
        "refData",
        JSON.stringify({ senderObjectId, receiverId })
      );
  ...
  const onSend = async () => {
    try {
      ...
      const receiverId = receiverId.replace(/\//g, "");
      ...
      const chatRef = doc(db, "chats", `${senderObjectId}_${receiverId}`);
